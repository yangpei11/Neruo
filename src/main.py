...
def parse_arguments(arguments=None):
    ...
    parser.add_argument('--useSimpleLabel', required=False, default=False, help='')
    parser.add_argument('--useDict', required=False, default=False, help='')
    try:
    ...
    print( arguments.get('train_model') )
    if arguments.get('train_model') == 'False':
        arguments['dataset_text_folder'] = '../unannotated_texts'
        for filename in os.listdir('../unannotated_texts/deploy'):
            #print( os.listdir('../unannotated_texts/deploy') )
          #  print( '')
            f = open('../unannotated_texts/deploy/' + filename, 'r', encoding = 'utf-8')
            text = f.read()
            f.close()
            if arguments.get('useDict') == 'True':
            ...
            for i in text:
                xx.append(i)
            text = ' '.join(xx)
            f = open('../unannotated_texts/deploy/' + filename, 'w', encoding = 'utf-8')
            f.write(text)
            f.close()

    arguments.pop('useDict')
    arguments.pop('useSimpleLabel')
    return arguments
...
def pro(filename, path, choose):
    ...
    f = open('../data/conll2003/en/'+choose + '/' + filename + '.ann', 'w', encoding = 'utf-8')
    f.write('\n'.join(m))
    f.close()
    f1 = open('../data/conll2003/en/'+choose + '/' + filename + '.txt', 'w', encoding = 'utf-8')
...
def Init(arguments=None):
    if arguments == None:
        return
    global xxxxxx
    if( arguments.get('useSimpleLabel') == 'True'):
        if os.path.exists('../zh/train') and os.path.exists('../zh/valid'):
            for filename in os.listdir('../data/conll2003/en/'):
                if( os.path.isfile('../data/conll2003/en/'+filename) ):
                    os.remove('../data/conll2003/en/'+filename)
                elif( os.path.isdir('../data/conll2003/en/'+filename) ):
                    shutil.rmtree('../data/conll2003/en/'+filename)
            os.mkdir("../data/conll2003/en/train") 
            os.mkdir("../data/conll2003/en/valid")

            for filename in os.listdir('../zh/train'):
                pro(filename, '../zh/train/'+filename, 'train')
            for filename in os.listdir('../zh/valid'):
                pro(filename, '../zh/valid/'+filename, 'valid')
            f = open('../dic.txt', 'w', encoding = 'utf-8')
            f.write( '\n'.join(xxxxxx) )
            f.close()

        else:
            print("please check train files or valid files exists all in zh Floder")
